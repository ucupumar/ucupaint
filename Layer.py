...
def open_images_to_single_layer(context, directory:str, import_list:list[str], texcoord_type, uv_name,
        add_mask=False, mask_type='IMAGE', mask_color='BLACK', mask_use_hdr=False, 
        mask_uv_name = '', mask_width=1024, mask_height=1024, use_image_atlas_for_mask=False, use_udim_for_mask=False) -> bool:
    ...
    for i, image in enumerate(valid_images):
        ...
        if i == 0:
            ...
            layer = add_new_layer(node.node_tree, image.name, 'IMAGE', 
                    int(ch_idx), 'MIX', 'MIX', 
                    normal_map_type, texcoord_type, uv_name, image, None, None,
                    (1,1,1),
                    add_mask, mask_type, mask_color, mask_use_hdr, 
                    mask_uv_name, mask_width, mask_height, use_image_atlas_for_mask, 
