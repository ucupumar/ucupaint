...
def draw_layers_ui(context, layout, node):
    ...
    if layer:
        ...
        if is_bl_newer_than(4, 3) and in_texture_paint_mode:
            brush = context.tool_settings.image_paint.brush
            if ((mask_image and mask.socket_input_name == 'Color') or override_image) and (brush.name in tex_eraser_asset_names or brush.blend == 'ERASE_ALPHA'):
                bbox = col.box()
                row = bbox.row(align=True)
                row.alert = True
            ...
            if ((mask_image and mask.socket_input_name == 'Color') or override_image) and brush.name == eraser_names[obj.mode]:
                bbox = col.box()
                row = bbox.row(align=True)
                row.alert = True
                row.operator('paint.y_toggle_eraser', text='Disable Eraser')
            ...
            if mask_vcol and mask.socket_input_name == 'Color' and brush.name == eraser_names[obj.mode]:
                bbox = col.box()
                row = bbox.row(align=True)
                row.alert = True
                row.operator('paint.y_toggle_eraser', text='Disable Eraser')
