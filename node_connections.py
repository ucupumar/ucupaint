...
def reconnect_source_internal_nodes(layer, used_output_names=[]):
    ...
    for outp in get_available_source_outputs(source, layer_type):
        ...
        if outp.name in used_output_names:
            if used_output_idx < len(layer.mod_groups):
                mod_group = tree.nodes.get(layer.mod_groups[used_output_idx].name)
            used_output_idx += 1
...
def reconnect_mask_source_nodes(mask, layer_tree):
    ...
    if start:
        ...
        elif mask.use_baked or mask.type not in {'VCOL', 'HEMI', 'OBJECT_INDEX', 'BACKFACE', 'EDGE_DETECT', 'AO', 'MODIFIER'}:
            create_link(tree, start.outputs[0], source.inputs[0])
...
def reconnect_layer_nodes(layer, ch_idx=-1, merge_mask=False):
        ...
        if layer.texcoord_type == 'UV':
            ...
            if texcoord:
                ...
                if decal_process: 
                    ...
                    if layer_decal_scale_y: create_link(tree, layer_decal_scale_y, decal_process.inputs[3])
    ...
    for i, mask in enumerate(layer.masks):
            ...
            if mask.use_baked or mask.texcoord_type == 'UV':
                ...
                if mask_texcoord:
                    ...
                    if mask_decal_process: 
                        ...
                        layer_decal_scale_x = get_essential_node(tree, TREE_START).get(get_entity_input_name(mask, 'decal_scale_x'))
                        layer_decal_scale_y = get_essential_node(tree, TREE_START).get(get_entity_input_name(mask, 'decal_scale_y'))
                        ...
                        if layer_decal_scale_y: create_link(tree, layer_decal_scale_y, mask_decal_process.inputs[3])
        ...
        if mask_uv_neighbor:
            ...
            if not mask.use_baked and mask.type in {'VCOL', 'HEMI', 'OBJECT_INDEX', 'COLOR_ID', 'BACKFACE', 'EDGE_DETECT', 'AO', 'MODIFIER'}:
