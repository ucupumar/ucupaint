    ...
    for i, ch in enumerate(yp.channels):
        ...
        if end_linear and (end_linear.type != 'GROUP' or end_linear.node_tree):
            if ch.type == 'NORMAL':
                ...
                if height: height = create_link(tree, height, end_linear.inputs['Height'])[1]
                if height_n: create_link(tree, height_n, end_linear.inputs['Height N'])
                if height_s: create_link(tree, height_s, end_linear.inputs['Height S'])
                if height_e: create_link(tree, height_e, end_linear.inputs['Height E'])
                if height_w: create_link(tree, height_w, end_linear.inputs['Height W'])
            ...
            if ch.type == 'NORMAL':
                ...
                if ch.enable_subdiv_setup:
                    if end_normal_engine_filter:
                        ...
                        else: break_input_link(tree, end_normal_engine_filter.inputs['Cycles'])
        ...
        if root_ch.type == 'NORMAL':
            ...
            if height_blend:
                if not root_ch.enable_smooth_bump:
                    ...
                    if alpha_n: create_link(tree, alpha_n, height_blend.inputs['Alpha N'])
                    if alpha_s: create_link(tree, alpha_s, height_blend.inputs['Alpha S'])
                    if alpha_e: create_link(tree, alpha_e, height_blend.inputs['Alpha E'])
                    if alpha_w: create_link(tree, alpha_w, height_blend.inputs['Alpha W'])
                    ...
                    if normal_proc:
                        ...
                        if 'Height N' in normal_proc.inputs: create_link(tree, height_blend.outputs['Height N'], normal_proc.inputs['Height N'])
                        if 'Height S' in normal_proc.inputs: create_link(tree, height_blend.outputs['Height S'], normal_proc.inputs['Height S'])
                        if 'Height E' in normal_proc.inputs: create_link(tree, height_blend.outputs['Height E'], normal_proc.inputs['Height E'])
                        if 'Height W' in normal_proc.inputs: create_link(tree, height_blend.outputs['Height W'], normal_proc.inputs['Height W'])
