...
def transfer_uv(objs, mat, entity, uv_map, is_entity_baked=False):
    ...
    temp_objs = []
    if len(objs) > 1 and not is_joining_objects_problematic(yp):
...
def get_number_of_bake_passes(yp, only_active_channel):
    num_passes = 0

    # Get related channels first
    chs = []
    ...
    if only_active_channel:
        ...
        for channel in yp.channels:
            ...
            # Do not add channel without layers
            if not is_any_layer_using_channel(channel, node):
                continue

            chs.append(channel)

    # Add color/alpha channel pair
    if color_ch in chs and alpha_ch not in chs:
        chs.append(alpha_ch)

    if alpha_ch in chs and color_ch not in chs:
        chs.append(color_ch)
    ...
    for ch in chs:
        ...
        # NOTE: Normal channel currently always bake extra data
        if ch.type == 'NORMAL':
            if is_baked_normal_without_bump_needed(ch): num_passes += 1
            if any_layers_using_disp(ch): num_passes += 1
            if any_layers_using_vdisp(ch): num_passes += 1
    ...
    if mat.users > 1:
        ...
        # Check if joining objects possible
        problems = is_joining_objects_problematic(yp, mat, return_reasons=True)
        if any(problems):
            objs = get_all_objects_with_same_materials(mat, True)
            num_passes *= len(objs)
    
    return num_passes, problems
...
class YBakeChannels(bpy.types.Operator, BaseBakeOperator):
    ...
    def invoke(self, context, event):
        ...
        num_bake_iterations = 1

        # Check if there are multiple objects
        self.num_bake_pass, self.bake_problems = get_number_of_bake_passes(yp, self.only_active_channel)
    ...
    def draw(self, context):
        ...
        if any(self.bake_problems):
            col = self.layout.column(align=True)
            col.alert = True
            col.label(text='Bake passes needed: '+str(self.num_bake_pass), icon='ERROR')
            #col.label(text='Multiple objects using the same material found!', icon='ERROR')
            col.label(text='Remove any of the following to lower the passes:', icon='BLANK1')
            for reason in self.bake_problems:
                col.label(text=reason, icon='DOT')
            col.alert = False
    ...
    def execute(self, context):
        ...
        temp_objs = []
        ori_objs = []
        if (len(objs) > 1 or any_uv_geonodes) and not is_joining_objects_problematic(yp, mat):
